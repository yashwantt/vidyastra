  ...
  >
    <el-table-column label="Class" min-width="200">
      <template slot-scope="scope">
        <a target="_blank" :href="'/#/attendance/mark?lookFor='+scope.row.class+''">{{ scope.row && scope.row.class }}</a>
      </template>
    </el-table-column>
...
export default {
  ...
  created() {
    ...
    fetchUser(this.$store.getters.userId)
      .then(response => {
        this.alloted = JSON.parse(response.data[0]['alloted']);
          ...
          data.records.forEach(element => {
            ...
            if (parseInt(myclass[0]) > settings['schools'][settings.myurl]['sections']['lg']){
              subjects.forEach(elemen => { // subjectSections
                if (elemen != 'All' && this.alloted.includes(myclass[0] + ' | ' + elemen)){
              ...
              settings['schools'][settings.myurl]['sections']['lt'].forEach(elemen => { // subjectSections
                if (elemen != 'NA' && this.alloted.includes(myclass[0] + ' | ' + elemen)) {
