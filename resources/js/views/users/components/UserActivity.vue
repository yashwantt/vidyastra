  ...
  <el-card v-if="user.name">
    <el-tabs v-model="activeActivity" @tab-click="handleClick">
      ...
      <el-tab-pane v-loading="updating" label="Account" name="third">
        ...
        <el-form-item label="Change Password (optional)">
          <el-input v-model="user.password" type="password" :disabled="user.role === 'admin'" />
        </el-form-item>
        <el-form-item v-if="getRole()" label="Class/Subject  Alloted" prop="alloted"><br>
          <el-select v-model="user.alloted" style="width:100%" filterable allow-create default-first-option multiple @change="handleAlloted">
            <el-option v-for="_class in classes" :key="_class" :label="_class" :value="_class" />
          </el-select>
        </el-form-item>
        <el-form-item v-if="getRole()" label="Alloted Options" prop="allotedOptions"><br>
          <el-checkbox-group v-model="user.checkedAllotedOptions" @change="handleAllotedOptions">
            <el-checkbox v-for="opt in user.allotedOptions" :key="opt" style="width:100%;" :label="opt">{{ opt }}</el-checkbox><br>
          </el-checkbox-group>
        </el-form-item>
        <el-form-item>
...
export default {
  ...
  methods: {
    ...
    handleAlloted(value){
      this.user.allotedOptions = [];

      settings.allotedOptions.forEach(element => {
        value.forEach(elemen => {
          this.user.allotedOptions.push(element + ' of ' + elemen);
          // this.user.checkedAllotedOptions.push(element+ " of "+elemen);
        });
      });
    },
    handleAllotedOptions(value){
      const checkedCount = value.length;
      this.checkAll = checkedCount === this.user.allotedOptions.length;
      this.isIndeterminate = checkedCount > 0 && checkedCount < this.user.allotedOptions.length;
      // console.log(this.user.checkedAllotedOptions);
    },
