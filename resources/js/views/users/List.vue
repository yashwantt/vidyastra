<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-select v-model="listQuery.role" :placeholder="$t('table.role')" clearable style="width: 90px" class="filter-item" @change="handleFilter">
    ...
    <pagination v-show="total>0" small layout="total, prev, pager, next" :page.sync="listQuery.page" :limit.sync="listQuery.limit" :total="total" @pagination="getList" />
    ...
    <el-dialog :title="'Create new user'" :visible.sync="dialogFormVisible">
      <div v-loading="userCreating" class="form-container">
        <el-form ref="userForm" :rules="rules" :model="newUser" label-position="top" label-width="200px" style="max-width: 500px;">
          <el-tabs v-model="activeName">
            <el-tab-pane label="Login Detail" name="login">
              <el-form-item :label="$t('user.role')" prop="role">
                <el-select v-model="newUser.role" class="filter-item" placeholder="Please select role" @change="handleUserChange">
              ...
              <el-form-item :style="newUser.seen" label="Class/Subject Alloted" prop="alloted">
                <el-select v-model="newUser.alloted" style="width:100%" filterable allow-create default-first-option multiple @change="handleAlloted">
                  <el-option v-for="_class in classes" :key="_class" :label="_class" :value="_class" />
                </el-select>
              </el-form-item>
              <el-form-item :style="newUser.seen" label="Alloted Options" prop="allotedOptions"><br>
                <el-checkbox-group v-model="newUser.checkedAllotedOptions" @change="handleAllotedOptions">
                  <el-checkbox v-for="opt in newUser.allotedOptions" :key="opt" :label="opt">{{ opt }}</el-checkbox><br>
                </el-checkbox-group>
              </el-form-item>
            </el-tab-pane>
          </el-tabs>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      nonAdminRoles: settings.nonAdminRoles,
      newUser: {},
      classes: [],
      ...
      subjects: settings['schools'][settings.myurl]['subjects'],
  ...
  methods: {
    ...
    resetNewUser() {
      this.newUser = {
        ...
        seen: 'display:block',
        allotedOptions: [],
        checkedAllotedOptions: [],
        checkAll: false,
      };
    },
    handleAlloted(value){
      this.newUser.allotedOptions = [];

      settings.allotedOptions.forEach(element => {
        value.forEach(elemen => {
          this.newUser.allotedOptions.push(element + ' of ' + elemen);
        });
      });
    },
    handleAllotedOptions(value){
      const checkedCount = value.length;
      this.checkAll = checkedCount === this.newUser.allotedOptions.length;
      this.isIndeterminate = checkedCount > 0 && checkedCount < this.newUser.allotedOptions.length;
      // console.log(this.newUser.checkedAllotedOptions);
    },
    handleDownload() {
